...
impl Index {
  ...
  pub fn get_collections_paginated(
    ...
    // 收集所有唯一的父序列号
    // key 格式: parent_sequence_number (8 bytes) + child_sequence_number (8 bytes)
  ...
  pub fn get_children_by_sequence_number_paginated(
    ...
    // 使用前缀迭代，只查找以 parent_sequence_number 开头的键
    // key 格式: parent_sequence_number (8 bytes) + child_sequence_number (8 bytes)
    let prefix = sequence_number.to_be_bytes();
    // 使用 IteratorMode::From 直接从目标前缀开始迭代，避免遍历整个表
    ...
    while let Some(Ok((key_bytes, _))) = iter.next() {
      // 检查 key 是否以指定的父序列号开头
      // 如果前缀不匹配，说明已经遍历完所有匹配的项（因为键是排序的）
      if key_bytes.len() < 8 || key_bytes[0..8] != prefix {
        break; // 前缀不匹配，说明已经遍历完所有相关项
      }

      // 从 key 的后8字节提取子序列号
      if key_bytes.len() < 16 {
        continue; // key 格式不正确，跳过
      }
  ...
  pub fn get_home_inscriptions(&self) -> Result<Vec<InscriptionId>> {
    // For RocksDB, we'll use direct database access instead of transactions
    let home_inscriptions_cf = self
      .database
      .cf_handle(CF_HOME_INSCRIPTIONS)
      .ok_or_else(|| anyhow!("Column family 'home_inscriptions' not found"))?;

    let mut inscriptions = Vec::new();
    let mut iter = self
      .database
      .iterator_cf(home_inscriptions_cf, IteratorMode::End);

    while let Some(Ok((_number_bytes, id_bytes))) = iter.next() {
      let id = InscriptionId::load(id_bytes.to_vec());
  ...
  pub fn get_address_info(&self, address: &Address) -> Result<Vec<OutPoint>> {
    ...
    // 使用 IteratorMode::From 直接从目标 script_pubkey 前缀开始迭代，避免遍历整个表
    // key 格式: script_pubkey + outpoint (序列化)
    let mut iter = self.database.iterator_cf(
    ...
    while let Some(Ok((key_bytes, _))) = iter.next() {
      // 检查 key 是否以指定的 script_pubkey 开头
      // 如果前缀不匹配，说明已经遍历完所有匹配的项（因为键是排序的）
...
mod tests {
  ...
  fn old_schema_gives_correct_error() {
    let tempdir = {
      ...
      wtx
        .open_table(CF_STATISTIC_TO_COUNT)
        .unwrap()
