...
impl SimpleHttpTransport {
        ...
        R: for<'a> serde::de::Deserialize<'a>,
    {
        ...
        let read_len = match content_length {
            None => FINAL_RESP_ALLOC,
        ...
        let mut body_buf = Vec::with_capacity(read_len as usize);
        let mut body_reader = sock.take(read_len);
        body_reader.read_to_end(&mut body_buf)?;

        if let Some(expected_len) = content_length {
            if (body_buf.len() as u64) != expected_len {
                return Err(Error::HttpResponseContentLengthTooLarge {
                    length: body_buf.len() as u64,
                    max: expected_len,
                });
            }
        }
