...
use crate::network_info::{GetBlockchainInfoResult, GetNetworkInfoResult};
...
pub trait RpcApi: Sized {
  ...
  fn get_block(&self, hash: &bitcoin::BlockHash) -> Result<Block> {
    let hex_str: String = self.call("getblock", &[into_json(hash)?, 0.into()])?;
  ...
  fn get_block_header_info(&self, hash: &bitcoin::BlockHash) -> Result<json::GetBlockHeaderResult> {
  ...
  fn get_raw_mempool_verbose(&self) -> Result<HashMap<bitcoin::Txid, json::GetMempoolEntryResult>> {
  ...
  fn get_node_addresses(&self, count: Option<usize>) -> Result<Vec<json::GetNodeAddressesResult>> {
